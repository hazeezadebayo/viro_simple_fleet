    ...
    def on_timer(self, r_id=0):
                ...
                if r_id != 0:
                        ...
                        try:    # show all robot's in this fleet on the fleet map.
                                ...
                                print(error)
                        print("map for robot with id: "+str(r_id_)+", in fleet: "+str(fleet_id)+", is selected for use.")
                # plot the world
                self.get_world()
                # plot the car
                self.nav_plot()
...
# ----------------------------------------------------------------------------------------------------
    def get_world(self):
        if (self.nav_map_obtained == False) and (self.feedback_base_map_img is not None) and (self.feedback_base_map_img != 'None'):
                ...
                print("No image found. please ensure that fleet_map_path in client is properly set.")
                return
...
# ----------------------------------------------------------------------------------------------------
    def fm_send_task_request(self, from_loc_id, to_loc_id, task_name, task_priority):
                ...
                if robot_id == 0:
                        print("ATTENTION! Robot is not selected. Please choose robot.")
                        return
                ...
                if len(checkpoints) != 0:
                        if task_cleared == True:
                                ...
                                print('ATTENTION! Task not cleared. Hence, not assigned.')
...
# ----------------------------------------------------------------------------------------------------
    def request_tasks(self, from_loc_id, to_loc_id, task_name, task_priority):
        ...
        if robot_id == 0:
              print("Robot is not selected. Please choose robot.")
              return

        if from_loc_id == to_loc_id and (task_name != 'charge' and task_name != 'move'):
              print("'load' location cannot be equal to 'unload' location.")
              return

        if (from_loc_id not in self.job_ids) or (to_loc_id not in self.job_ids):
                print("only ids within this fleet ["+str(fleet_id)+"] form a valid task request.")
                return
        ...
        print("robot at: ", self.feedback_current_pose[0], self.feedback_current_pose[1], self.feedback_current_pose[2])
        for i in range(0,len(task_dictionary["itinerary"])):
                if task_dictionary["itinerary"][i]["fleet_id"] == fleet_id:
                        ...
                        if d < shortest_distance_found:
                                ...
                                print("found shortestest dist: ", shortest_distance_found," with loc: ", loc_node_owner)
        ...
        if task_name == 'charge' or task_name == 'move':
                ...
                if task_name == 'move':
                        if loc_node_owner != to_loc_id:
                                checkpoints = self.shortest_path(graph, loc_node_owner, to_loc_id) # print('imaginary_checkpoints: ', imaginary_checkpoints)
        ...
        print(" checkpoints to pass: ", checkpoints)
        print(" landmark to visit: ", landmark)
                ...
                if ((checkp_pos_[0] == checkp_pos_[1] == checkp_pos_[2]) or (agv_stat_[0] == 'idle')):
                        ...
                        if (float(battery_stat_[-1]) > 35) or (float(battery_stat_[-1]) <= 35 and task_name == 'charge'):
                                ...
                                if len(self.available_robots) > 0:
                                        print("Robot battery percent is less than 35%. Only 'charge' task is allowed. Please consider robot "+str(self.available_robots[0])+".")
                                else:
                                        print('ATTENTION! '+" battery percent is less than 35%. only 'charge' task is allowed.")
                else:
                        # robot is not available
                        task_cleared = False
                        self.available_robots = self.suggest_robot()
                        if len(self.available_robots) > 0:
                                print('ATTENTION! '+'Robot unavailable. Robot is currently active. Please cancel current task and reassign or consider robot '+str(self.available_robots[0])+'.')
                        else:
                                print('ATTENTION! '+'Robot unavailable. Robot is currently active. Please cancel current task and reassign.')
        except ValueError:
                # handle the error here, for example:
                print("Error: could not convert battery stat to float")
    ...
    def suggest_robot(self):
        ...
        try:
                ...
                for record in self.cur.fetchall():
                        ...
                        checkp_p_ = str(record['checkpoints']).split(',');# print(len(self.checkpoints),"-route: ", self.checkpoints) # [A,B,C,D]
...
# ----------------------------------------------------------------------------------------------------
    def fm_add_landmark_request(self, station_type, input_loc_id, neighbor_loc_ids): # limit by graph size, make only numeric
        ...
        print("location or landmark to save: \n")
        ...
        if not pattern.match(input_loc_id):
                print('ATTENTION! ' + 'Location Number 0 cannot be chosen. Start numbering from 1.\
                ...
                for i in range(0,len(task_dictionary["itinerary"])):
                        if task_dictionary["itinerary"][i]["fleet_id"] == fleet_id:
                                if task_dictionary["itinerary"][i]["loc_id"] == input_loc_id:
                                        print('ATTENTION! '+'Location ID already exists.')
                                        return
        ...
        for element in edges:
                if not pattern.match(element.strip()):  # strip() is used to remove leading/trailing spaces
                        print('ATTENTION! ' + "Invalid element in 'Neighbors' field: " + element + ".")
                        return
                if element not in self.landmark_dict['t']:
                        print('ATTENTION! ' + "Element " + element + " in 'Neighbors' field not a landmark.")
                        return
    ...
    def add_location_db_cmd(self, edges, x, y, w, z, loc_type, node, fleet_id):
        ...
        time.sleep(0.5)
        print('Add landmark completed successfully! '+'modify edges in the output config file. [yaml]!')
...
# ----------------------------------------------------------------------------------------------------
    def fm_clear_task_trigger(self, trigger:bool):
        ...
        if self.feedback_checkpoints[0] == 'A' or  self.feedback_checkpoints[0] == 'A' or self.feedback_checkpoints[0] == 'A':
               print('ATTENTION! '+ "Robot not currently executing any task. Operation not performed.")
               return
...
# ----------------------------------------------------------------------------------------------------
    ...
    def robot_status_db_cmd(self, agv_stat, robot_id, fleet_id):  # active/inactive - agv_stat
        ...
        try:
            ...
            self.conn.commit()
            print('Robot Status State: ', str(agv_stat))
        except Exception as error:
            print(error)
        ...
        try:
                ...
                for record in self.cur.fetchall():
                        ...
                        if (trigger is False) and self.m_controls[0] == 'yes':
                                ...
                                print('Success! manual drive mode is turned off.')
...
# ----------------------------------------------------------------------------------------------------
    ...
    def shutdown_db_cmd(self, shutdown, robot_id, fleet_id):  # yes/no - shutdown
        try:
            ...
            self.conn.commit()
            print('Robot Power State: ', str(shutdown))
        except Exception as error:
            print(error)
...
# ----------------------------------------------------------------------------------------------------
    def fm_delete_landmark_request(self, str_location_id_comma_fleet):
        ...
        if (loc_ == None) or (fleet_ == None):
                print('ATTENTION! '+ "Invalid element in 'delete landmark' field.")
                return
                ...
                for record in self.cur.fetchall():
                        ...
                        if notifications_[0] != notifications_[1]:
                                ...
                                message = client.messages.create(
                                                to = to_number,
                                                from_ = from_number,
                                                body = "Hello! Robot "+r_id_+" in fleet "+fleet_id+" is lost. Will try reversing. Please inspect. ")
