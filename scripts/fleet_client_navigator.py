...
class FleetClientNavigator(Node):
    ...
    def __init__(self):
        ...
        self.simple_fleet_notice_pub = self.create_publisher(String,'/'+package_name+'/external_notifice', 1)
        self.marker_publisher = self.create_publisher(MarkerArray,"/cpp_markers", 1)

        self.notification_stat_sub = self.create_subscription(String,'/'+package_name+'/internal_notifice', self.simple_fleet_int_cb, 1, callback_group=self.group)
        ...
        while self.count < len(self.inspection_points):
            ...
            if i % 10 == 0:
                ...
                if self.landmarks[1] != "clean":
                    ...
                    self.get_logger().info(" [client] ----- xxxxx published xxxxxxxx ------ : ["+str(0)+"]. \n")
                else:
                ...
                if result is True:
                    ...
                    if self.client_plugin.recovery_retry_counter == 0:
                        self.get_logger().info(" [client] ------------------- : ["+str(result)+"]. \n")
                        if len(self.negotiated_route) != 0:
                                ...
                                if (landmark == self.checkpoints[self.count:][0]) and \
                                    ...
                                    if status is True:
                                        ...
                                        if self.landmarks[1] == "charge" or self.landmarks[1] == "move":
                                            ...
                                            self.get_logger().info(" [client] -----------move task completed-------- . \n")
...
# ----------------------------------------------------------------------- #
    ...
    def simple_fleet_ext_pub(self, notification):
        ...
        self.get_logger().info(" [client] -----extpub xxxooxxx ------ : ["+str(self.msg_notice.data)+"]. \n")
